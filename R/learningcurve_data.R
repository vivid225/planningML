...
#'
#' @import caret doParallel parallel
#' @importFrom pROC roc
...
learningcurve_data = function(x, y, method="log", metric="MCC", target=NULL, batchsize = 60, class.prob,
  ...
  train_index = createDataPartition(dt_full$y, p = 0.2, list = FALSE)
  train_df <- dt_full[train_index,]
  test_df <- dt_full[-sample(train_index,1000),]
  ...
  for (i in 1:floor(nrow(train_df)/batchsize)){
    ...
    for (j in 1:nrepeat){
      ...
      control <- trainControl(search = "random",
                              ...
                              summaryFunction = MySummary, allowParallel = TRUE)
